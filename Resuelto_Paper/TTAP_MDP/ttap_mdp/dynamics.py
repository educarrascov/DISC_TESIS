...
class Simulator:
    ...
    def _sample_release_times(self) -> dict[str, float]:
        ...
        for task in self.scenario.tasks:
            ...
            if (
                ...
                and self.rng.random() > config.initial_task_fraction
            ):
                release = max(
                    release,
                    self.rng.uniform(0.0, config.dynamic_arrival_window),
                )
    ...
    def _travel_multiplier(self) -> float:
        ...
        return exp(self.rng.normalvariate(mean, sigma))
    ...
    def _sample_travel_time(
        ...
        return nominal * self._travel_multiplier()
    ...
    def _dispatch_assignment(self, action: Action) -> None:
        ...
        if self.rng.random() < self.uncertainty.failure_probability:
            downtime = self.rng.uniform(
