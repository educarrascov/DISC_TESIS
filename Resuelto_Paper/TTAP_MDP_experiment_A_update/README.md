...
- familia anidada del Experimento A con 4 áreas, 4 helicópteros y
  20/30/40/50/60 tareas;
...
## Experimento A dinámico: Greedy online vs. PPO

El notebook:

```text
notebooks/02_experiment_a_scaling_greedy_vs_ppo.ipynb
```

usa exactamente la geometría y la flota del Experimento A. Entrena un PPO por
tamaño sobre instancias `1001–1060` y reserva las 30 instancias `1–30` para
evaluación fuera de muestra. Incluye modo piloto y formal, diferencias pareadas,
intervalos de confianza, estabilidad entre semillas, resultados por prioridad
y episodios operacionales detallados. La curva de aprendizaje se presenta solo
como diagnóstico, no como evidencia de superioridad frente a Greedy.
