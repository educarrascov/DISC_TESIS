...
class DynamicsTests(unittest.TestCase):
    ...
    def test_travel_stream_is_independent_of_unrelated_dispatch_order(self) -> None:
        uncertainty = UncertaintyConfig(travel_time_cv=0.2)
        first = Simulator(self.scenario, uncertainty, seed=218)
        second = Simulator(self.scenario, uncertainty, seed=218)

        first.step(first.action_index(ActionKind.ASSIGN, "H1", "T1"))
        second.step(second.action_index(ActionKind.ASSIGN, "H2", "T2"))
        second.step(second.action_index(ActionKind.ASSIGN, "H1", "T1"))

        first_flight = next(
            record.flight_time
            for record in first.log
            if record.event == "task_dispatched"
            and record.helicopter_id == "H1"
            and record.task_id == "T1"
        )
        second_flight = next(
            record.flight_time
            for record in second.log
            if record.event == "task_dispatched"
            and record.helicopter_id == "H1"
            and record.task_id == "T1"
        )
        self.assertEqual(first_flight, second_flight)
